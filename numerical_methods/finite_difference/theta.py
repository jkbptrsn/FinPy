...
class Theta:
    ...
    def __init__(self,
        ...
        self._xmin = xmin
        self._xmax = xmax
        self._nstates = nstates + 2  # Adding boundary states
        self._dt = dt
        self._theta = theta
        ...
        # Only needed in AndersenPiterbarg1D
    ...
    def nstates(self) -> int:
        return self._nstates - 2  # Removing boundary states
...
class AndersenPiterbarg1D:
    """Solve parabolic 1-factor PDE.
    The general structure of the PDE is
    dV/dt + drift * dV/dx + 1/2 * diffusion^2 * dV^2/dx^2 = rate * V
    The numerical solution is determined using the theta method:
        - theta = 0   : Explicit Euler
...
class AndersenPiterbarg1DTest(Theta):
    """Solve parabolic 1-factor PDE.
    The general structure of the PDE is
    dV/dt + drift * dV/dx + 1/2 * diffusion^2 * dV^2/dx^2 = rate * V
    The numerical solution is determined using the theta method:
