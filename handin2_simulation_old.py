    ...
    for n in range(1, n_steps):
        ...
        if mode == 'analytic':
            ...
            spot_temp, payoff, delta = \
                data_set(option, n * time_step, n_paths, delta_mode=delta_mode)
            poly_price, poly_delta = \
                polynomials(poly_order, spot, spot_temp, payoff, delta, w=w)
            a = poly_delta
        b = V - a * spot
...
#    error = (V - np.maximum(spot - strike, 0)) / V_0
    error = (V - np.maximum(spot - strike, 0))
    error_mean = sum(error) / n_paths
#    error_std = math.sqrt(sum((error - error_mean) ** 2) / n_paths)
    error_std = math.sqrt(sum((error - error_mean) ** 2) / n_paths) / (sum(V_0) / n_paths)
...
for w in w_vector:
    for idx_p, p in enumerate(poly_vector):
        if math.fabs(w - 1.0) < 0.1:
            plt.plot(n_sim_vector, price_only[idx_p, :], '-xy')
        else:
            plt.plot(n_sim_vector, price_delta[idx_p, :], '-ob')
